/**
 * Testhilfen für den Zustand des juris-SH-Adapters: temporäre Roots (nie der echte Bestand) und ein
...
import type { ManifestEntry } from '@landesrecht/importer-juris-sh/common/manifest.ts';
import { manifestEntryDefaults } from '@landesrecht/importer-juris-sh/common/manifest.ts';
import type { ValidityEvidence } from '@landesrecht/importer-juris-sh/common/evidence.ts';

const roots: string[] = [];

/** Temporäres Repository-Root; wird von `cleanupTempRoots()` wieder entfernt. */
export async function tempRoot(prefix = 'landesrecht-juris-sh-'): Promise<string> {
...
export const SAMPLE_IDENTITY = 'jlr-TestgesetzSH2020rahmen';
...
export function sampleManifestEntry(overrides: Partial<ManifestEntry> = {}): ManifestEntry {
  return {
    ...
    sourceTitle: 'Testgesetz für das Land Schleswig-Holstein',
    sourceType: 'gesetz',
    sourceUrl: 'https://example.invalid/jportal/testgesetz',
    sourceVersion: { url: 'https://example.invalid/jportal/testgesetz', validFrom: '2020-01-01', validTo: null },
    selectedVersionUrl: 'https://example.invalid/jportal/testgesetz',
    sourceValidFrom: '2020-01-01',
    sourceValidTo: null,
    baselineStatus: 'active-at-baseline',
    baselineRecoveryMethod: 'historical-juris-version',
    ...
    transformerVersion: 'juris-sh-transformer/1.0.0',
    targetSlug: 'testg-nsh',
