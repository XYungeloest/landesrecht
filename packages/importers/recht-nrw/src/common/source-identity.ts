...
export interface RechtNrwVersionAddress {
  ...
  /** Geltungsbeginn der Fassung aus dem Pfad (ISO-Datum); fehlt bei undatierten LRMB-Altdatensätzen. */
  pathDate?: string;
  slug: string;
}

/** `/lrgv|lrmb/<typ>/<TTMMJJJJ>-<slug>`; im Bereich LRMB auch undatiert (`/lrmb/<typ>/<slug>`, SMBl-Altdatensätze). */
...
export function parseVersionUrl(value: string): RechtNrwVersionAddress | null {
  ...
  if (!year && section === 'lrgv') return null;
  const address: RechtNrwVersionAddress = {
  ...
  if (year) address.pathDate = `${year}-${month}-${day}`;
  return address;
}
