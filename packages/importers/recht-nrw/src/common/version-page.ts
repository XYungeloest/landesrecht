...
export interface RechtNrwVersionPage {
  ...
  /** Einzelne Einträge der Änderungshistorie (redaktionelle Hinweise, Fundstellenverlauf, Veröffentlichungen). */
  changeHistoryItems: string[];
...
export function parseVersionPage(html: string, requestedUrl: string): RechtNrwVersionPage {
  ...
  if (!address) throw new Error(`${requestedUrl}: keine RECHT.NRW-Fassungsadresse (/lrgv/<typ>/<TTMMJJJJ>-<slug> oder /lrmb/<typ>/[<TTMMJJJJ>-]<slug>)`);
  ...
  const undatedRecord = address.section === 'lrmb' && !address.pathDate;
  if (!validFrom) findings.push(undatedRecord ? { severity: 'warning', code: 'undated-record-without-validity', message: 'Undatierter LRMB-Datensatz ohne „Gültig ab“ (Geltung nur über Belege bestimmbar)' } : { severity: 'error', code: 'missing-valid-from', message: 'Infobox nennt kein „Gültig ab“' });
  if (validFrom && address.pathDate && validFrom !== address.pathDate) findings.push({ severity: 'warning', code: 'path-date-mismatch', message: `„Gültig ab“ ${validFrom} weicht vom Datum im Pfad ${address.pathDate} ab` });
  ...
  if (versions.length === 0) findings.push(undatedRecord ? { severity: 'warning', code: 'undated-record-without-version-list', message: 'Undatierter LRMB-Datensatz ohne Fassungsliste' } : { severity: 'error', code: 'missing-version-list', message: 'Keine Fassungsliste gefunden' });
  if (versions.length > 0 && !versions.some((entry) => entry.isCurrentPage)) findings.push({ severity: 'warning', code: 'current-page-not-marked', message: 'Die Fassungsliste markiert die aktuelle Seite nicht' });
  ...
  const page: RechtNrwVersionPage = { address, title, versions, attachments, content, infoFields, changeHistoryItems, findings };
