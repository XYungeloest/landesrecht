/**
 * Dokumentierte redaktionelle Overrides des NSH-Bestands (`data/imports/juris-sh/overrides.json`).
 *
 * Einzelfälle (widersprüchliches „gültig bis“, falsch zugeordnete Fundstelle, strittige Normativität)
 * werden nie im Code behandelt, sondern hier mit Wert, Begründung, Beleg und Prüfdatum festgehalten.
 * Die Importpfade wenden ausschließlich Overrides dieser Datei an und schreiben sie ins Manifest.
 *
 * Eigene Datei statt Wiederverwendung der West-Overrides: Die Felder sind portalspezifisch (NSH kennt
 * zusätzlich `baselineRecoveryMethod` und `sourceProvenance`), die Quellidentität hat ein anderes Format,
 * und die Bestände bleiben strikt getrennt.
 */
import { join } from 'node:path';

import { readJsonFile, writeJsonAtomic } from '@landesrecht/importer-recht-nrw/common/atomic.ts';

import { INSTITUTION_STATUSES } from './constants.ts';
import { BASELINE_RECOVERY_METHODS, DIGITAL_REPRESENTATIONS, PUBLICATION_AUTHORITIES } from './manifest.ts';
import { OVERRIDES_PATH } from './paths.ts';

export const OVERRIDES_SCHEMA = 'juris-sh-overrides/1' as const;
...
export interface OverrideEvidence {
  /** Art des Belegs, z. B. „Fassungsliste des Portals“, „GVOBl. Schl.-H.“. */
  source: string;
  url?: string;
  sha256?: string;
  note?: string;
