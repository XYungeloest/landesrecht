/**
 * Importmanifest des juris-SH-Adapters: ein Eintrag je Stammnorm, eine Datei je Eintrag.
 *
 * Ablage (bulkfähig, wie West): `data/imports/juris-sh/manifest/<bereich>/<identität>.json`. Nach jeder
 * verarbeiteten Stammnorm wird genau diese Datei atomar geschrieben; ein Abbruch beschädigt keine anderen
 * Einträge. Laufmetadaten (`importedAt`, `runId`) ändern sich nur, wenn sich der fachliche Inhalt ändert –
 * ein Wiederholungslauf mit unveränderten Quellen erzeugt keinen Diff.
 *
 ...
 * hier neu geschrieben. Übergangslösung bis zu einem dritten Adapter, siehe Modulkopf von `environment.ts`.
 *
 * Gegenüber West zusätzlich (Besonderheit Schleswig-Holstein/juris):
 *   baselineRecoveryMethod  Wie der Stichtagsstand gewonnen wurde. juris zeigt regelmäßig nur die aktuelle
 *                           Fassung; der Stichtag 2023-12-01 muss deshalb oft über historische Fassungen,
 *                           das Zurücknehmen späterer Änderungen oder aus Verkündungen gewonnen werden.
...
import { compareSourceIdentity, identityFileName, manifestAreaDir, manifestEntryPath } from './paths.ts';

export const MANIFEST_SCHEMA = 'juris-sh-import-manifest/1' as const;
export const MANIFEST_ENTRY_SCHEMA = 'juris-sh-import-manifest-entry/1' as const;

/** Statuswerte wie West – Auswertungen über beide Bestände bleiben vergleichbar. */
...
/**
 * Wie der Stichtagsstand gewonnen wurde (NSH-spezifisch):
 *   current-source                die abgerufene Quellfassung gilt am Stichtag selbst
 *   historical-juris-version      eine im Portal geführte historische Fassung deckt den Stichtag
 ...
 */
export const BASELINE_RECOVERY_METHODS = ['current-source', 'historical-juris-version', 'reverse-post-baseline-event', 'reconstructed-from-publications'] as const;
...
export interface ManifestEntry {
  ...
  /** NSH-spezifisch: Weg, auf dem der Stichtagsstand gewonnen wurde. */
  baselineRecoveryMethod: BaselineRecoveryMethod;
  /** NSH-spezifisch: Amtlichkeit und Digitalisierungsgrad der benutzten Darstellung. */
