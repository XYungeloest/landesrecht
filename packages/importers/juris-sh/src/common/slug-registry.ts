/**
 * Dauerhafte Slugvergabe des NSH-Bestands (`data/imports/juris-sh/slug-registry.json`).
 *
 * Verfahren wie im West-Adapter (eigener Zustand, eigene Jurisdiktionskonvention):
 ...
 *
 * Jurisdiktionskonvention: Der Landeszusatz des Simulationslandes ist immer `-nsh`. Die reale Quellabkürzung
 * „SH“ (Schleswig-Holstein) darf nie in einen Slug gelangen – ein Kandidat auf `-sh` oder
 * `-schleswig-holstein` wird auf `-nsh` überführt, bevor er reserviert wird. Slugs ohne Landeszusatz
...
export const SLUG_REGISTRY_SCHEMA = 'juris-sh-slug-registry/1' as const;
/** Landeszusatz des Simulationslandes; niemals `-sh`. */
 ...
 * Landeszusatz der Quelle durch `-nsh` ersetzen. Ein bereits korrekter Kandidat bleibt unverändert.
 */
export function jurisdictionSlugCandidate(candidate: string): string {
  ...
  const withJurisdiction = normalized
    .replace(/-schleswig-holstein$/u, JURISDICTION_SUFFIX)
    .replace(/-sh$/u, JURISDICTION_SUFFIX);
  if (/-sh$/u.test(withJurisdiction)) throw new Error(`Slug ${withJurisdiction} trägt den Quellzusatz -sh statt ${JURISDICTION_SUFFIX}`);
...
export function validateSlugRegistry(registry: SlugRegistry, path = SLUG_REGISTRY_PATH): void {
  ...
  for (const entry of registry.entries) {
    ...
    if (/-sh$/u.test(entry.slug)) throw new Error(`${path}: Slug ${entry.slug} trägt den Quellzusatz -sh statt ${JURISDICTION_SUFFIX}`);
