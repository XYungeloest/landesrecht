/**
 * Review-Queue des juris-SH-Adapters.
 *
 * Ablage: eine Datei je Stammnorm unter `data/imports/juris-sh/review/<bereich>/<identität>.json` –
 * strikt getrennt von der West-Queue (`data/imports/recht-nrw/review/**`). Zusätzlich trägt jeder Fall
 * `jurisdiction: 'nsh'` und `sourceSystem: 'juris-sh'`; eine Vermischung beider Bestände ist damit auch
 * dann erkennbar, wenn Fälle einmal zusammen ausgewertet werden.
 *
 * Verfahren wie im erprobten West-Adapter (bewusst gleiche Semantik, eigener Zustand):
...
export const REVIEW_QUEUE_SCHEMA = 'juris-sh-review-queue/1' as const;
export const REVIEW_SHARD_SCHEMA = 'juris-sh-review-shard/1' as const;

/**
 * Kategorien des NSH-Bestands. Gegenüber West neu sind `reconstruction-required` als eigene Kategorie
 * (juris zeigt regelmäßig nur die aktuelle Fassung), `contradictory-evidence` (Belege widersprechen
 * einander, Regel C in `evidence.ts`) und `import-regression` (ein bereits übernommener Eintrag
 * verschlechtert sich in einem Wiederholungslauf).
 */
...
export interface ReviewItemInput {
  ...
  /** Fachlicher Schlüssel innerhalb der Kategorie (z. B. „baseline-gap“, „ministry“). */
  key: string;
...
export interface ReviewDecision {
  ...
  /** Kennung des dokumentierten Overrides (`data/imports/juris-sh/overrides.json`). */
  override?: string;
}

export interface ReviewItem extends ReviewItemInput {
  id: string;
  /** Simulationsjurisdiktion des Falls – immer `nsh`. */
  jurisdiction: typeof TARGET_JURISDICTION;
  /** Quellsystem des Falls – immer `juris-sh`. */
...
/** Review-Status einer Stammnorm für das Manifest (gleiche Regel wie West). */
