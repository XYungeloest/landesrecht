/**
 * Transformation Schleswig-Holstein → Land Niedersachsen-Holstein (`nsh`).
 *
 * Ablauf wie beim erprobten RECHT.NRW-Transformer: (1) Erlassorgan nur aus ausdrücklicher Formel
 * bestimmen (organs.ts), (2) alle landesbezogenen Bezeichnungen auf dem unveränderten Quelltext
 * erkennen und entscheiden (detection.ts, zentrale Institutionen-Zuordnung), (3) nur die
 * Landesbezeichnung nach benannten Regeln überleiten (rules.ts), (4) nach der Transformation
 * prüfen: Residuen, Doppelbildungen, nicht angewandte Regeln, stille Änderungen.
 *
...
export function transformToNsh(law: SourceLaw, context: TransformContext, options: TransformOptions = {}): TransformResult {
  ...
  const version: NormVersion = parseNormVersion({
    ...
    changeNote: `Ausgangsfassung zum Ausgangsrechtsstand ${context.baselineDate}: übernommener Rechtsstand des Landes ${SOURCE_STATE} (Quellfassung gültig ${sourceInterval}), übergeleitet auf ${TARGET.name}.${options.provenanceNote ? ` ${options.provenanceNote}` : ''}`,
    sourceReferences: law.sourceReferences,
