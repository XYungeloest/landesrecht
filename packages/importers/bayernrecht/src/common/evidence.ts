/**
 * Evidenzmodell des BAYERN.RECHT-Adapters: reine Datenstrukturen, Validierung und Entscheidungsregeln.
 *
 * Muster sind der Evidence Pass des West-Imports und das Evidenzmodell des NSH-Adapters; dieses Modul
 * ist wie jene portalunabhängig: Es kennt keine Seite von BAYERN.RECHT, kein HTML und keine
 * Erkennungsregel. Es beantwortet nur die Frage, was eine Menge von Belegen über den Stichtag aussagt.
 * Die portalspezifische Erkennung (welcher Seitenbestandteil welchen Beleg erzeugt) entsteht später im
 * Parser-/Enumerationsstrang.
 *
 * Dimensionen (worüber ein Beleg etwas aussagt):
 *   begin      Geltungsbeginn der Vorschrift oder der Fassung
 *   end        Außerkrafttreten, Ablauf, Aufhebung durch die Vorschrift selbst (in Bayern häufig als
 *              befristete Geltung mit ausdrücklichem Ablaufdatum)
 ...
 *
 * Beweisklassen (identisch zu West und NSH, damit Auswertungen über alle Bestände vergleichbar bleiben):
 ...
 * Normtext, Index), nicht wie die Quelle ihn darstellt. `registry-position` ist bayerisch: die
 * Führung einer Vorschrift unter einer BayRS-Gliederungsnummer ist selbst ein Geltungsindiz –
 * für sich genommen aber nie stark genug, um allein zu entscheiden.
 */
...
/** Ergebnis der Stichtagsprüfung; gleiche Werte wie im West- und NSH-Manifest. */
...
export interface ValidityEvidence {
  ...
  sourceUrl?: string;
  sha256?: string;
  /** Fundstelle im amtlichen Verkündungsblatt (in Bayern GVBl., AllMBl., BayMBl.). */
  citation?: string;
...
export function validateValidityEvidence(value: unknown, where = 'Beleg'): string[] {
  ...
  // Die bloße Führung in der Rechtssammlung datiert nichts und entscheidet nichts.
  if (evidence.kind === 'registry-position' && evidence.strength === 'strong') problems.push(`${where}: registry-position ist höchstens stützend, nie stark`);
  return problems;
}
