/**
 * Transformation Bayern → Freistaat Bayern-Württemberg (`baywue`).
 *
 * Ablauf wie beim erprobten RECHT.NRW- und beim juris-SH-Transformer: (1) Erlassorgan nur aus
 * ausdrücklicher Formel bestimmen (organs.ts), (2) alle landesbezogenen Bezeichnungen auf dem
 * unveränderten Quelltext erkennen und entscheiden (detection.ts, zentrale Institutionen-Zuordnung),
 * (3) nur die Landesbezeichnung nach benannten Regeln überleiten (rules.ts), (4) nach der
 * Transformation prüfen: Residuen, Doppelbildungen, nicht angewandte Regeln, stille Änderungen.
 *
 * Quellmetadaten bleiben unverändert: Quellen-URLs, SHA-256, reales Quellintervall, reale Fundstelle
 * (`sourceCitation`), Fußnoten/Quellhinweise, Änderungshistorie, BayRS-Nummer
 * (`externalIdentifiers`) und das historische Erlassorgan (`originEnactingBody`). Die
 * Simulationsfundstelle (`citation`, `initialCitation`) ist davon getrennt und enthält keine reale
 * Fundstelle.
 */
...
import { SOURCE_STATE, TARGET_JURISDICTION, type SourceArea } from '../common/constants.ts';
import { assertJurisdictionSlug, jurisdictionSlugCandidate, JURISDICTION_SUFFIX } from '../common/slug-registry.ts';
...
import { adjectiveDecision, applySegments, planTransformation, targetProperName, transformationRules, TRANSFORMER_VERSION, type TransformationOptions } from './rules.ts';
...
export interface TransformationReport {
  schemaVersion: 'bayernrecht-transformation-report/1';
  source: string;
  ...
  rules: string[];
  /** Die redaktionelle Festlegung zur Adjektivbildung, maschinenlesbar (siehe rules.ts). */
  editorialDecisions: Array<{ id: string; source: string; chosen: string; rejected: string; reason: string }>;
...
export interface TransformOptions {
  sourceArea?: SourceArea;
  /** Kopfzeilen außerhalb des Normkörpers (z. B. Erlasskopf „Bekanntmachung des Staatsministeriums …“). */
 ...
 *
 * Die Jurisdiktionskonvention prüft und setzt die Zustandsschicht (`common/slug-registry.ts`):
 * `jurisdictionSlugCandidate` entschärft den Kandidaten und überführt einen realen Landeszusatz
 * (`-bayern`, `-bay`, `-by`) in `-baywue`; `assertJurisdictionSlug` lässt einen verbliebenen
 * Quellzusatz hart scheitern statt ihn still zu korrigieren. Ein Slug, der bereits auf den
 * ausgeschriebenen Zielnamen endet (`…-bayern-wuerttemberg`), bekommt keinen zweiten Zusatz.
 *
 * Der Präfix „bay“ bayerischer Abkürzungen (BayBO → `baybo-baywue`) bleibt stehen: Abkürzungen
 * werden nicht übergeleitet (docs/BAYERN_TRANSFORMATION.md), und die Konvention verbietet nur den
 * Landeszusatz am Ende.
 */
export function deriveSlug(transformedAbbr: string | undefined, transformedShortTitle: string | undefined, transformedTitle: string): string {
  const raw = transformedAbbr ? slugify(transformedAbbr) : transformedShortTitle ? slugify(transformedShortTitle) : truncateSlug(slugify(transformedTitle), 72);
  const base = jurisdictionSlugCandidate(raw);
  const stateSlug = slugify(targetProperName());
  if (base.endsWith(JURISDICTION_SUFFIX) || base.endsWith(stateSlug)) return base;
  return assertJurisdictionSlug(`${base}${JURISDICTION_SUFFIX}`, 'Abgeleiteter Slug');
}

export function transformToBayWue(law: SourceLaw, context: TransformContext, options: TransformOptions = {}): TransformResult {
  const changes: TransformationChange[] = [];
  const findings: ImportFinding[] = [];
  const ruleOptions = options.transformation ?? {};
  if (context.targetJurisdiction !== TARGET_JURISDICTION) throw new Error(`Der BAYERN.RECHT-Transformer bedient nur ${TARGET_JURISDICTION}, nicht ${context.targetJurisdiction}`);
  ...
  if (!postTransformAudit.ok) {
    ...
    findings.push({
      severity: 'error',
      code: 'post-transform-audit',
      message: `Prüfung nach der Transformation fehlgeschlagen: ${unexplained.length} unerklärte Landesbezeichnung(en)${unexplained.length ? ` (${unexplained.slice(0, 3).map((entry) => `${entry.path}: „${entry.term}“`).join('; ')})` : ''}, ${postTransformAudit.doubledNames.length} Doppelbildung(en), ${postTransformAudit.unappliedTransforms.length} Regelabweichung(en), ${postTransformAudit.unrecordedChanges.length} nicht protokollierte Änderung(en)`,
    });
  }

  const slug = assertJurisdictionSlug(context.reserveSlug(deriveSlug(abbr, shortTitle, title)), 'Reservierter Slug');
  ...
  const version: NormVersion = parseNormVersion({
    ...
    changeNote: `Ausgangsfassung zum Ausgangsrechtsstand ${context.baselineDate}: übernommener Rechtsstand des Freistaates ${SOURCE_STATE} (Quellfassung gültig ${sourceInterval}), übergeleitet auf ${TARGET.name}.${options.provenanceNote ? ` ${options.provenanceNote}` : ''}`,
    sourceReferences: law.sourceReferences,
    ...
    entries: [
      {
        ...
        type: 'initial',
        title: `Ausgangsfassung zum Ausgangsrechtsstand ${formatBaseline(context.baselineDate)} (Rechtsüberleitung ${SOURCE_STATE} → ${TARGET.name}).`,
        citation: simulationCitation,
  ...
  const report: TransformationReport = {
    ...
    sourceArea: options.sourceArea ?? 'landesrecht',
    ...
    editorialDecisions: [adjectiveDecision()],
    detections,
    ...
    protectedFields: ['meta.sourceReferences', 'meta.sourceCitation', 'meta.originEnactingBody', 'meta.externalIdentifiers', 'version.sourceReferences', 'version.sourceNotes', 'version.sourceValidFrom', 'version.sourceValidTo', 'version.sourceCitation', 'version.changeNote', 'history.entries[0].note'],
  };
